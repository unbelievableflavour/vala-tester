...
public class HeaderBar : Gtk.HeaderBar {
    ...
    private ValaVersionManager vala_version_manager = ValaVersionManager.get_instance ();
    ...
    HeaderBar () {
        ...
        if (settings.get_string ("vala-version") == "" && vala_version_manager.get_versions ().length != 0) {
            settings.set_string ("vala-version", vala_version_manager.get_versions ()[0]);
        }
    ...
    private void generate_versions_dropdown () {
        ...
        for (int i = 0; i < vala_version_manager.get_versions ().length; i++) {
            Gtk.TreeIter iter;
            liststore.append (out iter);
            liststore.set (iter, Column.VERSION, vala_version_manager.get_versions ()[i]);
        }
    ...
    void item_changed (Gtk.ComboBox combo) {
        settings.set_string ("vala-version", vala_version_manager.get_versions () [combo.get_active ()]);
    }

    private void get_active_dropdown_index_and_set () {
        for (int i = 0; i < vala_version_manager.get_versions ().length; i++) {
            if (vala_version_manager.get_versions ()[i] == settings.get_string ("vala-version")) {
